...
|---------------|------------------|---------------------|-----------------|
| Parent & child account data | Until account deletion | User-initiated delete or **`LEGAL_REVIEW_REQUIRED`** admin suspension policy | Hard delete per `account.js` |
...
| `analytics_daily_snapshots` | Indefinite aggregate | No family_id | Anonymised aggregates |
...
| `contact_message` (support) | **`LEGAL_REVIEW_REQUIRED`** | Not deleted on family delete | Retained |
| `admin_audit_log` | **`LEGAL_REVIEW_REQUIRED`** | Not deleted on family delete | Retained |
...
| Newsletter opt-out tokens | Until unsubscribe + **`LEGAL_REVIEW_REQUIRED`** | `email_subscriptions` | **`LEGAL_REVIEW_REQUIRED`** |
| PIN audit log | Until account deletion | Child scoped | Deleted with child |
| DB deploy snapshots (ops) | Ops policy **`LEGAL_REVIEW_REQUIRED`** | VPS `data/deploy/snapshots` | Not user data export |
...
Public Privacy Notice must disclose exceptions — **`LEGAL_REVIEW_REQUIRED`** exact wording for IE.

---

## Review

Update when: new tables, new marketing tools, or statutory retention advice from counsel.
