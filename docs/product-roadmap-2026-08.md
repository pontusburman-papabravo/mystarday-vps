...
| R4.5 Native snabb-completion (widget) | **ENGINEERING COMPLETE** (rollout OFF) | Prod deploy **PASS** @ `7f0edfe8` (health SHA aligned); `native_widget_enabled` / `widget_completion_enabled` / `trusted_device_v1` global **OFF** | `r45-release-acceptance-contracts`; native compile **BLOCKED** (no Xcode/Android tree in CI VM); pilot acceptance **BLOCKED** pending store binaries + physical QA |
