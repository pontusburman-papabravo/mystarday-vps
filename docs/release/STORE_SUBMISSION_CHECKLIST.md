...
- [ ] **Screenshots replaced on the new version** — MANUAL  
  PASS criterion: the **new** version page (currently 1.4.6 or later) shows the intended screenshot set. Screenshots on an already approved/live version cannot be swapped; a new version is required. That is why replacements did not appear on public 1.4.3, and cannot be swapped on approved 1.4.4 / 1.4.5 either.
...
- [ ] **Screenshots reflect the current build on both stores** — MANUAL
  PASS criterion: no UI shown in a screenshot has since changed materially; capture fresh screenshots after any significant UI change (see `docs/app-store-screenshots/README.md` for the capture process). On Apple, replace them on the **new version page** (§0) — not by editing an already-approved set.
