...
- **Repo bump ≠ store submission.** Bumping `MARKETING_VERSION` to **1.4.5** only prepares the next native archive. Tag `ios-v*` and Submit only when a native binary is actually required (plugins, permissions, entitlements, screenshot replacement after Ready for Distribution, or Apple requires a new binary).
- **Closed trains:** `1.4.3`, **`1.4.4`**. **Next native train:** `1.4.5`. Do not upload build 1188 or any other `1.4.4` binary. Do not reuse rejected build **1182**.
...
1. **Prepare marketing version** — only when a native binary is required: `npm run ios:xcode-cloud:version -- 1.4.5` (updates `MARKETING_VERSION` in `project.pbxproj`; does not bump `CURRENT_PROJECT_VERSION`). **1.4.4 and 1.4.3 are closed** (ITMS-90186 / ITMS-90062).
2. **PR + focused CI** — `npm run test:gate` and iOS release tests (see below).
3. **Merge to `main`**.
4. **Tag** — `ios-v1.4.5` on merged `main` (triggers Xcode Cloud archive when workflow is configured for `ios-v*` tags). Skip this step unless you intend to ship a native binary.
...
10. **App Store Connect version 1.4.5** — create this version only for a real native submission. Apple copies metadata from the current version. Verify Swedish + English (UK). Replace screenshots on **this** version (not on an already-approved page). Check Monthly/Yearly **ASC status**; add them to the same draft **only if not yet approved**.
