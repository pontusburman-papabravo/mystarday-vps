...
- `parent_activation_program` — 7-day parent activation experiment (status active|completed|opted_out|expired; cohort_arm treatment|control; program_type onboarding_7d; started_at, first_banner_seen_at, last_seen_day, day_status JSONB, reflection_score/text); unique partial index one active per family
- `parent_seen_completion` — parent aha-moment tracking (parent_id, daily_log_item_id, seen_at); PK (parent_id, daily_log_item_id)
...
- 2026-06-09: Feature — Föräldraaktivering 7D MVP Fas 1–4 (SW v204): `parent_activation_program` + `parent_seen_completion` tables; Luxon day logic (`calendar_day`/`effective_day`); A/B enroll on onboarding complete (post `ACTIVATION_PROGRAM_LAUNCH_AT`); treatment dashboard banner + day 1 inline preview; celebratory aha modal; reflection dag 7+; analytics funnel (`activation_program_started` → `first_banner_seen` → `child_first_completion` → `parent_first_completion_seen`). Control enrolled but no UI. Docs: `docs/foraldaraktivering-implementation-contract.md`. Feature slug: `foraldaraktivering_7d`. Env: `ACTIVATION_PROGRAM_ENABLED`, `ACTIVATION_PROGRAM_LAUNCH_AT`.
