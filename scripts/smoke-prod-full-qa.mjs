...
async function phaseBrowser() {
  ...
  async function assertOnPage(route, label) {
    ...
    try {
      await page.screenshot({
        path: path.join(ARTIFACTS, `parent-${label}.png`),
        fullPage: true,
        timeout: 60000,
        animations: 'disabled',
      });
    } catch (e) {
      record(cat, `screenshot ${route.path}`, false, e.message?.slice(0, 80) || 'timeout');
    }
  }
  ...
  record(cat, 'login role-selection hidden', !loginUi.roleSelection, loginUi.roleSelection ? 'still visible' : 'ok');
  ...
  if (!(await page.locator('#clKeypad').isVisible({ timeout: 8000 }).catch(() => false))) {
    ...
    await page.waitForSelector('#clKeypad', { state: 'attached', timeout: 15000 });
  }
  ...
  const childTabRoutes = { world: '/child/world', family: '/child/family' };
  for (const wid of ['world', 'family']) {
    ...
    if (await btn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      await page.goto(`${BASE}${childTabRoutes[wid]}`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    }
    await page.waitForTimeout(2000);
  ...
  try {
    await page.screenshot({
      path: path.join(ARTIFACTS, 'child-final.png'),
      fullPage: true,
      timeout: 60000,
      animations: 'disabled',
    });
  } catch (e) {
    record(cat, 'screenshot child-final', false, e.message?.slice(0, 80) || 'timeout');
  }
  await browser.close();
}
