...
import { verifyChecksumSidecar } from './lib/db-backup-core.mjs';

const TARGET_AGES_DAYS = [7, 14, 21, 27];

function pickHistoricalBackup(files, now = new Date()) {
  const candidates = files.filter((f) => f.kind === 'daily' || f.kind === 'predeploy' || f.kind === 'legacy_predeploy');
...
async function main() {
  ...
  const metaPath = chosen.path.replace(/\.dump$/, '.meta.json');
  ...
  writeBackupStatus({
    ...
    last_weekly_historical_target_days: TARGET_AGES_DAYS[
