 ...
 * post-migration with new migrations: fail-closed business-table + contract checks.
 * post-migration with zero new migrations: tolerate live business writes; protect identity, migration history, flags.
 */
import {
...
export function compareDbSnapshots(before, after, options = {}) {
  ...
  if (mode === 'post-migration') {
    ...
    const hasNewMigrations = names.length > 0;
    ...
    if (hasNewMigrations) {
      ...
      const beforeNames = new Set(before.applied_migration_names || []);
      const afterNames = after.applied_migration_names || [];
      if (afterNames.length !== beforeNames.size || afterNames.some((name) => !beforeNames.has(name))) {
        drift.push({
          issue: 'migration_history_changed',
          before_count: beforeNames.size,
