...
describe('Merge readiness — Migration 1808950000000 (child_progression_node authority)', () => {
  it('child_progression_node exists with PK, unique constraint, and idempotent unlock', async (t) => {
    ...
    try {
      ...
      const pk = await db.query(`
        ...
        ORDER BY a.attnum
      `);
      assert.deepEqual(pk.rows.map((r) => r.attname), ['id']);
      ...
      assert.ok(indexNames.some((n) => n.includes('child_progression_node_child_id_node_id')));
      ...
      await db.query(
        `INSERT INTO child_progression_node
         ...
         VALUES ($1, $2, 'routine_home', 'node_a', 'node', '', '{}'::jsonb)
         ON CONFLICT (child_id, node_id) DO NOTHING`,
        [child.rows[0].id, fam.rows[0].id]
      );
      const dup = await db.query(
        `INSERT INTO child_progression_node
           (child_id, family_id, world_slug, node_id, node_type, pack_config_key, metadata)
         VALUES ($1, $2, 'routine_home', 'node_a', 'node', '', '{}'::jsonb)
         ON CONFLICT (child_id, node_id) DO NOTHING RETURNING node_id`,
        [child.rows[0].id, fam.rows[0].id]
      );
  ...
  it('migration file exposes down() for rollback', () => {
    const mod = require('../../migrations/1808950000000_platform_runtime.js');
    assert.equal(mod.name, '1808950000000_platform_runtime');
