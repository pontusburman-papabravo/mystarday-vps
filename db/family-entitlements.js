...
async function upsertIntroYear(familyId, {
  client = null,
  startsAt = null,
  expiresAt = null,
  metadata = {},
  ...
  const { rows } = await q(
    `INSERT INTO family_entitlements (
     ...
     VALUES ($1, $2, 'intro_year', 'intro_year_grant', 'active', $3, $4, $5::jsonb)
     ON CONFLICT DO NOTHING
     RETURNING *`,
  ...
  const existing = await q(
    `SELECT * FROM family_entitlements
     WHERE family_id = $1 AND entitlement_key = $2 AND source = 'intro_year' AND revoked_at IS NULL
     LIMIT 1`,
    [familyId, PREMIUM_ENTITLEMENT_KEY]
  );
  return existing.rows[0] || null;
