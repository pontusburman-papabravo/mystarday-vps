...
/**
 * Child completion for widget (and reusable from other entry points).
 * @returns {Promise<{ status: 'completed'|'already_completed', justCompleted: boolean, star_value: number, completeRow: object|null }>}
 */
async function completeChildDailyLogItem({
  ...
  try {
    ...
    const result = await client.query(
      ...
      [dailyLogItemId, logDate, completionSource]
    );
    ...
    if (justCompleted) {
      ...
      handleActivityCompleted(dailyLogItemId, childId, false).catch((err) => {
        console.error('[WIDGET-COMPLETE] handleActivityCompleted failed:', err.message);
      });
      getChildFamilyId(childId).then(async (fid) => {
        ...
        require('./platform-runtime').handleActivityComplete({
          ...
          console.error('[platform-runtime] widget complete error:', err.message);
        });
      }).catch(() => {});
    }
