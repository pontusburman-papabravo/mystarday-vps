 ...
 * BILLING_READY             — store + RevenueCat + device paid path verified
 * READY_TO_OPEN             — explicit founder/ops approval to flip a market flag
 * PAID_ROLLOUT_READY        — billing ready + explicit paid-rollout approval
 *
...
function evaluateCountryReleaseGates(evidence, country) {
  ...
  const billingReady = isVerified(appleIap)
    && isVerified(playSkus)
    && isVerified(e.revenuecat)
    && isVerified(iosDevice)
    && e.android_sandbox_e2e !== 'MANUAL_VERIFICATION_REQUIRED'
    && e.apple_paid_download_unresolved_p0 !== true;

  // Explicit ops/founder approval. unit_tests_pass is ignored on purpose.
  const readyToOpen = prebillingMarketReady && isApproved(e[openKey]);
  const paidRolloutReady = billingReady && isApproved(e[paidKey]);

  return {
    country: cc,
    [GATE.CLOSED_CODE_READY]: closedCodeReady,
    [GATE.PREBILLING_MARKET_READY]: prebillingMarketReady,
    [GATE.BILLING_READY]: billingReady,
