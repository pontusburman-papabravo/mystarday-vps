...
const { JOURNEY_PUSH_LOCK_ID } = require('./scheduler-constants');
...
async function runJourneyPushJob() {
  ...
  const outcome = await withAdvisoryLock(JOURNEY_PUSH_LOCK_ID, async () => {
  ...
  if (outcome?.skipped === 'lock') {
    console.log('[journey-push] Skipping — another instance holds the lock');
  }
}
