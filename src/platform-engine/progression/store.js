...
class PgProgressionStore {
  ...
  async unlock(childId, worldSlug, nodeId, metadata = {}) {
    const familyRow = await this._db.query(
      'SELECT family_id FROM child WHERE id = $1 LIMIT 1',
      [childId]
    );
    const familyId = familyRow.rows[0]?.family_id;
    if (!familyId) {
      throw new Error(`PgProgressionStore.unlock: child ${childId} not found`);
    }

    const nodeType = metadata.node_type || 'node';
    const packConfigKey = metadata.pack_config_key || '';
    const { rows } = await this._db.query(
