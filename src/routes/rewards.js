...
parentRouter.post('/', validate(CreateRewardSchema), async (req, res) => {
  try {
    ...
    if (visible_to_children !== undefined) {
      const norm = await normalizeVisibleToChildren(db, req.user.familyId, visible_to_children);
      if (norm.error) {
        return res.status(400).json({ error: norm.error });
      }
      validatedVisible = norm.value;
    }
    const result = await db.query(
      ...
      [req.user.familyId, name.trim(), icon || '🎁', cost, requires_approval === true, validatedVisible]
    );
...
parentRouter.put('/:id', validateParams(UUIDParam), validate(UpdateRewardSchema), async (req, res) => {
  try {
    ...
    if (body.visible_to_children !== undefined) {
      const norm = await normalizeVisibleToChildren(db, req.user.familyId, body.visible_to_children);
      if (norm.error) {
        return res.status(400).json({ error: norm.error });
      }
      ...
      for (const item of order) {
        ...
        await client.query(
          `UPDATE reward_redemption SET sort_order = $1
           WHERE id = $2
           AND child_id IN (SELECT child_id FROM parent_child WHERE parent_id = $3 AND revoked_at IS NULL)`,
  ...
  try {
    ...
    const rrLookup = await client.query(
      `SELECT rr.id, rr.status, rr.child_id, rr.star_cost,
       ...
       WHERE rr.id = $1 AND pc.parent_id = $2 AND pc.revoked_at IS NULL`,
  ...
  try {
    ...
    const updated = await client.query(
      `UPDATE reward_redemption rr
       ...
       WHERE rr.id = $1
         ...
         AND pc.child_id = rr.child_id
         AND pc.parent_id = $2
         AND pc.revoked_at IS NULL
       RETURNING rr.id,
         COALESCE(rr.reward_name, (SELECT name FROM reward r WHERE r.id = rr.reward_id)) AS reward_name`,
      [req.params.id, req.user.id]
    );
    ...
    if (updated.rows.length === 0) {
      ...
      const exists = await client.query(
        `SELECT rr.status FROM reward_redemption rr
         JOIN parent_child pc ON pc.child_id = rr.child_id AND pc.parent_id = $2 AND pc.revoked_at IS NULL
         WHERE rr.id = $1`,
        [req.params.id, req.user.id]
      );
...
childRouter.post('/rewards/:id/redeem', async (req, res) => {
  ...
  let redeemRequiresApproval = true;
  ...
  try {
    ...
    const requiresApproval = reward.requires_approval === true;
    const redemptionStatus = requiresApproval ? 'pending' : 'auto';
    const redeemedAtSql = requiresApproval ? 'NULL' : 'NOW()';

    const insertResult = await client.query(
      `INSERT INTO reward_redemption (
         reward_id, child_id, status, star_cost, reward_name, reward_icon, redeemed_at
       )
       VALUES ($1, $2, $3, $4, $5, $6, ${redeemedAtSql})
       RETURNING id, status`,
      [req.params.id, childId, redemptionStatus, reward.star_cost, reward.name, reward.icon]
    );
    ...
    redeemRequiresApproval = requiresApproval;
  } catch (err) {
    await client.query('ROLLBACK').catch(() => {});
  ...
  const message = redeemRequiresApproval
  ...
  if (!redeemRequiresApproval) {
    return;
  }

  // Fire-and-forget: email + push notify linked parents (pending only)
