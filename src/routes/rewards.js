...
parentRouter.post('/', validate(CreateRewardSchema), async (req, res) => {
  try {
    ...
    // visible_to_children: null = all children, [] = no children, [uuid...] = specific
    let validatedVisible = null;
    if (visible_to_children === null || visible_to_children === undefined) {
      validatedVisible = null;
    } else if (Array.isArray(visible_to_children)) {
      ...
      if (childIds.length === 0) {
        ...
        if (validatedVisible.length === 0) validatedVisible = [];
      }
    }
    const result = await db.query(
