/**
 * Dashboard activation program banner (Fas 3 — onboarding_7d, treatment only).
 */
(function () {
  ...
  function bindReflectionHandlers() {
      ...
      submitBtn.addEventListener('click', async () => {
        ...
        try {
          const res = await window.apiFetch('/api/me/activation-program/reflection', {
            ...
            body: JSON.stringify({ score: selectedScore, text }),
          });
          if (res.ok) {
  ...
  async function handleCta(data) {
    ...
    if (content.cta_url && content.cta_type !== 'open_child_view') {
  ...
  function render(data) {
    ...
    let reflectionHtml = '';
    if (data.show_reflection && data.reflection_score == null) {
      reflectionHtml = renderReflection();
    }
    ...
    const showCta = content.cta_label && !data.show_reflection;
