...
(function () {
  ...
  function bindSubstepClicks(container) {
    ...
    container.addEventListener('click', function (e) {
      if (e.target.closest('.activity-timer-wrap, .substep-timer-slot')) return;
  ...
  function renderInteractiveSubsteps(container, itemId, steps) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      html +=
        '<div class="substep-row" id="substep-row-' + esc(step.id) + '">' +
        '<div class="substep-row-primary" role="button" tabindex="0" aria-pressed="' + (isChecked ? 'true' : 'false') + '" data-substep-item="' + esc(itemId) + '" data-substep-id="' + esc(step.id) + '" data-substep-done="' + (isChecked ? '1' : '0') + '">' +
