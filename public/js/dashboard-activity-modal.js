/**
 * Dashboard activity + template modal flow (Fas 8 F2e).
 * Add-activity modal, template list/search, recurrence modal, create/edit/delete item,
 * once-task flow — extracted verbatim from dashboard.js.
 * Reads/writes dashboard.js top-level state (allTemplates, selectedTemplateId,
 * addSectionOverride, editSectionVal, scheduleItems, currentChildId, currentDay, children, …)
 * and globals (DAYS, SECTIONS, fmtTime via ScheduleCore; selectChild, loadScheduleForDay,
 * renderSchedule, renderDayTabs, loadDashboardCards, showToast, apiFetch) via shared script scope.
 * Handlers are exposed on window for inline onclick + dashboard.js callers.
 */
(function () {
