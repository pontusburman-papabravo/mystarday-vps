...
async function openRequestPanel(childId, childName) {
  ...
  content.innerHTML = '<p class="text-center text-text-soft py-6">' + spt('home.approvals.loading') + '</p>';
  ...
  try {
    ...
    if (childRedemptions.length === 0 && childGoalChanges.length === 0) {
      content.innerHTML = '<p class="text-center text-text-soft py-6">' + spt('home.approvals.empty') + '</p>';
      return;
    }
      ...
      html += `
        <div class="bg-purple-50 border border-purple-200 rounded-xl p-4">
          <div class="flex items-center gap-2 mb-2">
            ...
            <div class="flex-1">
              <div class="font-heading font-bold text-sm text-navy">${spt('home.approvals.wantsGoalChange')}</div>
              <div class="text-xs text-text-soft">${spt('home.approvals.goalChangeTo', { name: escHtml(req.to_reward_name || '') })} ${req.to_reward_icon || ''}</div>
            </div>
          </div>
          <div class="flex gap-2 mt-3">
            <button onclick="event.stopPropagation(); approveGoalChange('${req.id}')" class="flex-1 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors min-h-[44px]">✅ ${spt('home.approvals.approve')}</button>
            <button onclick="event.stopPropagation(); denyGoalChange('${req.id}')" class="flex-1 bg-red-100 hover:bg-red-200 text-red-700 text-sm font-semibold py-2.5 rounded-xl transition-colors min-h-[44px]">❌ ${spt('home.approvals.deny')}</button>
          </div>
        </div>`;
    }
      ...
      html += `
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-4">
          <div class="flex items-center gap-2 mb-2">
            ...
            <div class="flex-1">
              <div class="font-heading font-bold text-sm text-navy">${spt('home.approvals.wantsRedeem')}</div>
              <div class="text-xs text-text-soft">${escHtml(req.reward_name || '')} (⭐ ${req.star_cost || 0})</div>
            </div>
          </div>
          <div class="flex gap-2 mt-3">
            <button onclick="event.stopPropagation(); approveRedemption('${req.id}')" class="flex-1 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors min-h-[44px]">✅ ${spt('home.approvals.approve')}</button>
            <button onclick="event.stopPropagation(); denyRedemption('${req.id}')" class="flex-1 bg-red-100 hover:bg-red-200 text-red-700 text-sm font-semibold py-2.5 rounded-xl transition-colors min-h-[44px]">❌ ${spt('home.approvals.deny')}</button>
          </div>
        </div>`;
    }
    ...
    content.innerHTML = '<p class="text-center text-red-500 py-6">' + spt('home.approvals.loadError') + '</p>';
  }
}
...
async function approveGoalChange(requestId) {
  try {
    ...
    if (!res.ok) { const e = await res.json(); showToast(e.error || spt('home.dayOffModal.genericError'), true); return; }
    showToast(spt('family.toasts.goalChangeApproved'));
    closeRequestPanel();
    await loadDashboardCards();
  } catch { showToast(spt('home.dayOffModal.networkError'), true); }
}
...
async function denyGoalChange(requestId) {
  try {
    ...
    if (!res.ok) { const e = await res.json(); showToast(e.error || spt('home.dayOffModal.genericError'), true); return; }
    showToast(spt('family.toasts.goalChangeDenied'));
    closeRequestPanel();
    await loadDashboardCards();
  } catch { showToast(spt('home.dayOffModal.networkError'), true); }
}
...
async function approveRedemption(redemptionId) {
  try {
    ...
    if (!res.ok) { const e = await res.json(); showToast(e.error || spt('home.dayOffModal.genericError'), true); return; }
    showToast(spt('family.toasts.redemptionApproved'));
    closeRequestPanel();
    await loadDashboardCards();
  } catch { showToast(spt('home.dayOffModal.networkError'), true); }
}
...
async function denyRedemption(redemptionId) {
  try {
    ...
    if (!res.ok) { const e = await res.json(); showToast(e.error || spt('home.dayOffModal.genericError'), true); return; }
    showToast(spt('family.toasts.redemptionDenied'));
    closeRequestPanel();
    await loadDashboardCards();
  } catch { showToast(spt('home.dayOffModal.networkError'), true); }
}
