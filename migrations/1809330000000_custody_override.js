...
/**
 * FEAT-1C — custody_override: date-range exceptions on top of grundschema.
 * Pipeline: override → pattern → fallback (ADR).
 */
  ...
  up: async (client) => {
    await client.query(`
      CREATE TABLE IF NOT EXISTS custody_override (
        ...
        CONSTRAINT custody_override_dates CHECK (start_date <= end_date)
      )
    `);
    await client.query(`
